<!-- GetCommentsAdmin -->
 <template>
    <div>
        <div class="Page">
            <a-page-header title="回复列表" :show-back="false">
                <template #subtitle>
                    <a-space>
            ...
            <div class="Pagecontent">
                <a-table :columns="columns" :data="UserList" :pagination="false" :loading="loading" :scroll="{
                    ...
                    <template #n_icon="{ record }">
                        <a-avatar :size="24">
                            <img alt="avatar" :src="record.user?.n_avatar" />
                        </a-avatar>
                    </template>
                    <template #n_nickname="{ record }">
                        <a-space>
                            <span>{{ record.user?.n_nickname }}</span>
                        </a-space>
                    </template>
...
const GetUserList = async () => {
    loading.value = true
    const res = await useApiFetch().post('/api/GetCommentsAdmin', GetFrom.value)
    if (res.code === 200) {
